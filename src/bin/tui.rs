...
enum Controller {
    Local { player: Player },
    Remote { socket: String, token: Option<String> },
}
...
impl Controller {
    ...
    async fn pause(&mut self) -> Result<()> {
        match self {
            ...
            Controller::Remote { socket, token } => { let _ = send_daemon_cmd(socket, token.as_deref(), "pause", None).await?; Ok(()) }
        }
    }
    ...
    async fn play_item(&mut self, item: &str) -> Result<()> {
        match self {
            ...
            Controller::Remote { socket, token } => { let _ = send_daemon_cmd(socket, token.as_deref(), "play", Some(item)).await?; Ok(()) }
        }
    }

    async fn enqueue(&mut self, item: &str) -> Result<()> {
        match self {
            Controller::Local { player } => { player.enqueue(item.to_string()); Ok(()) }
            Controller::Remote { socket, token } => { let _ = send_daemon_cmd(socket, token.as_deref(), "enqueue", Some(item)).await?; Ok(()) }
        }
    }
    ...
    async fn artist_info(&mut self, id: &str) -> Result<String> {
        match self {
            ...
            Controller::Remote { socket, token } => {
                let resp = send_daemon_cmd(socket, token.as_deref(), "artist_info", Some(id)).await?;
                if let Some(items) = resp.items { Ok(items.join("\n")) } else { Ok(resp.msg) }
            }
        }
    }
    ...
    async fn artist_discography(&mut self, id: &str) -> Result<String> {
        match self {
            ...
            Controller::Remote { socket, token } => {
                let resp = send_daemon_cmd(socket, token.as_deref(), "artist_discography", Some(id)).await?;
                if let Some(items) = resp.items { Ok(items.join("\n")) } else { Ok(resp.msg) }
            }
        }
    }
...
async fn send_daemon_cmd(socket: &str, token: Option<&str>, cmd: &str, arg: Option<&str>) -> Result<DaemonResp> {
    #[cfg(unix)]
    {
...
fn is_insecure_http(s: &str) -> bool { s.starts_with("http://") }
fn insecure_allowed() -> bool { std::env::var("APPLE_ALLOW_INSECURE").map(|v| v == "1" || v.eq_ignore_ascii_case("true")).unwrap_or(false) }

#[derive(Clone, Copy, Debug)]
enum Theme { Dark, Light }

impl Theme {
    fn from_env() -> Self { match std::env::var("APPLE_THEME").ok().as_deref() { Some("light") => Theme::Light, _ => Theme::Dark } }
    fn next(self) -> Self { match self { Theme::Dark => Theme::Light, Theme::Light => Theme::Dark } }
    fn header_style(self) -> Style { match self { Theme::Dark => Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD), Theme::Light => Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD) } }
    fn help_style(self) -> Style { match self { Theme::Dark => Style::default().fg(Color::Gray), Theme::Light => Style::default().fg(Color::DarkGray) } }
    fn list_highlight(self) -> Style { match self { Theme::Dark => Style::default().bg(Color::Blue).fg(Color::Black).add_modifier(Modifier::BOLD), Theme::Light => Style::default().bg(Color::Yellow).fg(Color::Black).add_modifier(Modifier::BOLD) } }
    fn modal_style(self) -> Style { match self { Theme::Dark => Style::default().fg(Color::White), Theme::Light => Style::default().fg(Color::Black) } }
}
...
async fn main() -> Result<()> {
    ...
    let mut controller = if let Ok(sock) = std::env::var("APPLE_DAEMON_SOCKET") {
        ...
        Controller::Remote { socket: sock, token }
    } else {
    ...
    // UI state
    let mut last_status = controller.status().await.unwrap_or_else(|_| "unknown".into());
    ...
    if let Some(ref t) = cfg.theme { theme = if t == "light" { Theme::Light } else { Theme::Dark } }
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            if prefs_open {
                ...
                let y = (size.height.saturating_sub(h)) / 2;
                let area = ratatui::layout::Rect::new(x, y, w, h);
                let options = vec![format!("Theme: {}", if let Theme::Light = theme { "Light" } else { "Dark" })];
                            ...
                            if let Some(action) = pending_artist_action {
                                if action == "info" {
                                    match controller.artist_info(&input_buf).await {
                                        Ok(info) => { modal_lines = info.lines().map(|s| s.to_string()).collect(); modal_scroll = 0; modal_open = true; }
                                        Err(_) => last_status = "failed to fetch artist info".into(),
                                    }
                                } else if action == "discography" {
                                    match controller.artist_discography(&input_buf).await {
                                        Ok(disc) => { modal_lines = if disc.is_empty() { vec!["<no albums>".into()] } else { disc.lines().map(|s| s.to_string()).collect() }; modal_scroll = 0; modal_open = true; }
                                        Err(_) => last_status = "failed to fetch discography".into(),
                                    }
                                }
                            } else {
                                if is_insecure_http(&input_buf) && !insecure_allowed() {
                                    last_status = "Refused insecure http URL; set APPLE_ALLOW_INSECURE=1 to allow".into();
                                } else {
                                    if input_enqueue { let _ = controller.enqueue(&input_buf).await; } else { let _ = controller.play_item(&input_buf).await; }
                                }
                            }
                            input_buf.clear(); mode_input = false; pending_artist_action = None;
                        }
                        KeyCode::Esc => { input_buf.clear(); mode_input = false; }
                        _ => {}
                    }
                } else if modal_open {
                    match code {
                        KeyCode::Up => { if modal_scroll > 0 { modal_scroll -= 1; } },
                        KeyCode::Down => { if modal_scroll + 1 < modal_lines.len() { modal_scroll += 1; } },
                        KeyCode::PageUp => { modal_scroll = modal_scroll.saturating_sub(10); },
                        KeyCode::PageDown => { modal_scroll = modal_scroll.saturating_add(10).min(modal_lines.len().saturating_sub(1)); },
                        KeyCode::Esc => { modal_open = false; modal_lines.clear(); modal_scroll = 0; },
                        _ => {}
                    }
                } else if prefs_open {
                    match code {
                        KeyCode::Up => { if prefs_selected > 0 { prefs_selected -= 1; } },
                        KeyCode::Down => { prefs_selected = prefs_selected.saturating_add(1).min(0); },
                        KeyCode::Enter => { theme = theme.next(); cfg.theme = Some(if let Theme::Light = theme { "light".into() } else { "dark".into() }); let _ = save_config(&cfg); },
                        KeyCode::Esc => { prefs_open = false; },
                        _ => {}
                    }
                } else {
                    match code {
                        KeyCode::Char('T') => { prefs_open = true; prefs_selected = 0; },
                        KeyCode::Char('t') => { theme = theme.next(); cfg.theme = Some(if let Theme::Light = theme { "light".into() } else { "dark".into() }); let _ = save_config(&cfg); },
                        KeyCode::Char('q') => break,
                        KeyCode::Char('p') | KeyCode::Char(' ') => { let _ = controller.pause().await; last_status = controller.status().await.unwrap_or_else(|_| "unknown".into()); },
                        KeyCode::Char('s') => { last_status = controller.status().await.unwrap_or_else(|_| "unknown".into()); },
                        KeyCode::Char('n') => { let _ = controller.next_and_play().await; last_status = controller.status().await.unwrap_or_else(|_| "unknown".into()); },
                        KeyCode::Char('a') => { mode_input = true; input_enqueue = false; pending_artist_action = None; input_buf.clear(); },
                        KeyCode::Char('e') => { mode_input = true; input_enqueue = true; pending_artist_action = None; input_buf.clear(); },
                        KeyCode::Char('i') => { mode_input = true; input_enqueue = false; pending_artist_action = Some("info"); input_buf.clear(); },
                        KeyCode::Char('d') => { mode_input = true; input_enqueue = false; pending_artist_action = Some("discography"); input_buf.clear(); },
                        KeyCode::Up => { if selected > 0 { selected -= 1; } },
                        KeyCode::Down => { if selected + 1 < queue.len() { selected += 1; } },
                        _ => {}
                    }
                }
            }
        ...
        if last_refresh.elapsed() > Duration::from_secs(2) { last_status = controller.status().await.unwrap_or_else(|_| "unknown".into()); last_refresh = Instant::now(); }
    }
